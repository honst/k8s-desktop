...
class Server(models.Model):
    ...
    asset = models.OneToOneField('Asset', null=True, on_delete=models.SET_NULL)  # 非常关键的一对一关联！
...
class CPU(models.Model):
    ...
    asset = models.OneToOneField('Asset', null=True, on_delete=models.SET_NULL)  # 设备上的cpu肯定都是一样的，所以不需要建立多个cpu数据，一条就可以，因此使用一对一。
...
class RAM(models.Model):
    ...
    asset = models.ForeignKey('Asset', null=True, on_delete=models.SET_NULL)  # 只能通过外键关联Asset。否则不能同时关联服务器、网络设备等等。
...
class NIC(models.Model):
    ...
    asset = models.ForeignKey('Asset', null=True, on_delete=models.SET_NULL)  # 注意要用外键
